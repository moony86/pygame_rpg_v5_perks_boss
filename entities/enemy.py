...
class Enemy:
    ...
    def _update_boss(self, distance, direction, dt):
        self.boss_phase = 2 if self.health < self.max_health * 0.50 else 1

        if self.attack_cooldown <= 0:
            attacks = ["boss_cone", "boss_ring", "boss_summon_leap"] if self.boss_phase == 1 else ["boss_ring", "boss_cone", "boss_summon_leap", "boss_ring"]
            action = attacks[self.boss_attack_index % len(attacks)]
            self.boss_attack_index += 1

            self._start_windup(0.95 if self.boss_phase == 1 else 0.72, direction, action)
            self.attack_cooldown = 2.1 if self.boss_phase == 1 else 1.55
            return
    ...
    def draw(self, surface, camera):
        ...
        pygame.draw.rect(surface, color, camera.apply_rect(self.rect), border_radius=7)
        ...
        if self.windup_timer > 0 and self.windup_direction.length_squared() > 0:
            ...
            if "ring" in action:
                pygame.draw.circle(surface, (255, 120, 40), (round(start.x), round(start.y)), 120, 4)
            elif "summon" in action:
                end = start + self.windup_direction * 290
                pygame.draw.line(surface, (255, 120, 40), start, end, 9)
                pygame.draw.circle(surface, (255, 120, 40), (round(start.x), round(start.y)), 32, 2)
            else:
                ...
                pygame.draw.circle(surface, (255, 120, 40), (round(start.x), round(start.y)), 22, 2)
